...
# the GitHub release ships a plain, self-contained binary per arch
# (local-ai-<version>-linux-<amd64|arm64>, no archive to extract) rather
# than a vendor install script, so resolve the latest release via the
# GitHub API and drop the binary straight into /usr/local/bin.
install_localai() {
...
# Zed (https://zed.dev) - GPU-accelerated code editor. zed-editor exists in
# the AUR, but this uses the vendor's own install script instead to match
# the other two distro scripts (three-script parity) - it drops the binary
# in ~/.local, so it must run as the invoking user rather than root, same
# shape as install_opencode/install_vibe_cli above.
install_zed() {
    local u="$SUDO_USER"; [ "$u" = "root" ] && u=""
    local check_cmd install_cmd
    if [ -n "$u" ]; then
...
# for Arch/AUR - Codeberg's own release assets include a self-contained
# Linux tarball (gram.app/ - bin/gram, libexec/gram-editor, bundled
# lib/*.so, share/applications/gram.desktop, share/icons/...) rather than a
# single binary, so the whole bundle is extracted into /opt (keeping
...
# rather than copied out alone. Version pinned to the one actually tested
# (3.3.0) rather than resolved via Codeberg's API at install time, unlike
# claude-desktop's "latest" above.
install_gram() {
...
# IDE. Not packaged for Arch/AUR - vendor curl|bash installer is genuinely
...
fix_elgato_wave3_profile() {
    ...
    mkdir -p "$conf_dir"
    if cat > "$conf" <<'WPEOF'
# Managed by post-install-arch.sh (fix_elgato_wave3_profile)
...
# github.com/itzender5820/mousiki - no AUR package, no binary release (the
# one GitHub release is a source zip) - the project's own setup.sh is
# source-build-only everywhere, so this mirrors that script's real Arch
# dependency list and build steps by hand rather than shelling out to it.
install_mousiki() {
    log INFO "Installing Mousiki build dependencies..."
    batch_install "Mousiki build deps" cmake base-devel ffmpeg yt-dlp python python-pip
    local t; t=$(mktemp -d)
    if ! git clone --depth 1 https://github.com/itzender5820/mousiki "$t/mousiki" 2>/dev/null; then
        rm -rf "$t"; log WARNING "Mousiki clone failed"; return 1
    fi
    ( cd "$t/mousiki" && cmake -B build 2>/dev/null && cmake --build build -j"$(nproc)" 2>/dev/null )
...
setup_mousiki_user_config() {
    ...
    local uh; uh=$(eval echo ~"$SUDO_USER" 2>/dev/null)
    [ -z "$uh" ] && return 0
    su - "$SUDO_USER" -c "pip install --user requests" &>/dev/null || true
...
show_main_menu() {
    ...
    if $IS_OMARCHY; then
        ui_cell "" "";                ui_cell 30 "Peripherals"; echo
    else
