...
| `Mod+Escape` | ASCII screensaver, on demand |
| `Mod+l` | Lock screen |
...
- **Idle-lock timing is fixed.** `xset s 1800 dpms 1800 3600 5400` shows the ASCII screensaver then locks the screen (via `xss-lock`) at 30 minutes idle, with the display standing by/suspending/powering off at 30/60/90 minutes as a fallback (the lock itself force-blanks the display immediately, regardless of these DPMS timers — see "ASCII screensaver" above) — edit that line in the generated `~/.config/i3/config` if you want different timings.
