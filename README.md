...
Everything above themes the desktop *session* — polybar/rofi/kitty only start after login. The screen shown before that, including the LUKS disk-decryption password prompt on an encrypted install, is rendered by **Plymouth**, a completely separate subsystem with its own theme format (an `ImageDir` of PNGs plus a `.plymouth` INI config, not anything resembling a polybar/rofi/kitty config). The script installs the real, official [catppuccin/plymouth](https://github.com/catppuccin/plymouth) theme's `catppuccin-mocha` flavor rather than a hand-built one — its background (`0x1e1e2e`) is the exact same Mocha base used everywhere else in this rice, and it uses Plymouth's standard `two-step` module and `Noto Sans`, both already present on a stock Fedora install, so no new package dependency beyond `git` (already installed above) is needed.

Since the theme's assets are binary PNGs, not text, it can't be written as a heredoc the way every other config here is — same as the Nerd Font, it's a runtime `git clone` into a temp directory, `sudo cp`'d into `/usr/share/plymouth/themes/catppuccin-mocha`, activated with `sudo plymouth-set-default-theme -R catppuccin-mocha` (the `-R` flag rebuilds the initramfs so it actually takes effect on next boot), wrapped in the same best-effort `if...then...else warn...fi` pattern as that font download so a network hiccup here can't abort the rest of the script.

**This could not be live-tested in this session** — applying and verifying it requires interactive `sudo` (no TTY/askpass available here) and a full reboot to actually see the LUKS decrypt screen, neither of which this session has access to. The theme comes from the real, official upstream repo and its `.plymouth` config was read directly to confirm it only depends on things already present on this system, but the actual boot-time rendering has not been visually confirmed — **reboot after running the script to see it**, and check the LUKS prompt specifically since that's the whole point of theming it.
...
- **[catppuccin/i3](https://github.com/catppuccin/i3)**, **[catppuccin/gtk](https://github.com/catppuccin/gtk)**, **[catppuccin/plymouth](https://github.com/catppuccin/plymouth)**: For the Catppuccin Mocha i3 border colors, GTK theme, and boot-splash/LUKS-prompt Plymouth theme
